...
**harness 维度**:多数条目是"换模型、harness 仍是 Claude Code";若本机装了 codex CLI,注册表会自动多出 `--codex`(`harness:"codex"`),它是**换整个 harness**——GPT 在 codex 自家工具循环里干活。选它的判据是"要 harness 差异/独立第二意见/GPT 原生 agentic",不是省钱(走订阅额度、不计入成本)。支持 `--with-context`(首次把当前对话导入成 codex thread,约 1-2s,会把主对话历史跨厂商外发给 OpenAI);限制:写模式默认关网络。详见 model-routing.md。
