...
2. **选模型**:用户显式指定或自然语言提名("用 GPT 改")就用之;没说就**不传**,让 runtime 用默认。本机装了 codex CLI 时 `--codex` 可用(走 codex harness,选它的判据见 `config/model-routing.md`)。**codex 写模式的两点差异务必知悉**:① 写边界由 codex 自己的 sandbox(`workspace-write`)定义,不是 gkd 的 allowed-tools 粒度;② **codex 写模式默认关闭网络**——装库/联网命令(npm install、git fetch 等)会失败,这类任务别派给 codex(或改用 Claude 系模型)。codex 也支持 `--with-context`(首次把当前对话导入成 codex thread,约 1-2s);但它会把主对话历史外发给 OpenAI(跨厂商,与 Claude 系不同),主对话含敏感内容时命中"codex + --with-context"宜用 `AskUserQuestion` 跟用户确认一次。
