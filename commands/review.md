...
- 否则估算:
  ...
  - **大小约 1-2 个文件且无目录级改动** → 推荐 `Wait for results`
  - 其他情况(包括估算不清)→ 推荐 `Run in background`
  - 拿不准时倾向"跑审查",而不是"没什么可审"
...
**后台(--background):**
用 Bash 工具的 `run_in_background: true` 启动同一条命令,然后告诉用户:
"GKD 审查已在后台启动。结果完成后我会通知你。"
不要在本轮等待 BashOutput。
...
- 若用户在 review 后说"帮我修一下",那时再调 `/gkd --write` 委派修复;`/gkd:review` 本身永远不修改。
